...
> **录制于 2026-09-14**，用当前构建：逻辑栈 + **4 台车 + 24 件物料**，取 220 s 长运行里
> 活动最密集的 **60 秒**、压成 90 帧、离屏渲染。这里画的是理想运动学，不是 Gazebo 接触。
>
> **这一版里没有小车卡死。** 早先那版会卡，原因在**厂房模型**而不是车队：
> 并条完工位与粗纱等料位之间只有 **0.65 m**，而两台车交会需要 **0.74 m** ——
> 一台车在那里停靠就永久堵死通道。现在工序净宽改为 **2.0 m**，实测卡死占比为 0；
> 对照数据见"稳定性"一节。
...
**死锁是厂房模型造成的，已经修好。** 车会卡在工位前的通道里。原因是几何：
并条完工位与粗纱等料位之间只有 **0.65 m**，而两台车通过需要
**0.44 + 2×0.05 = 0.74 m** 的车体净距。一台车在那里停靠就永久堵死通道，后面全排队。
