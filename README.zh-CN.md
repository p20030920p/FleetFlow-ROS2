...
现在改为两个有向矩形之间的**分离轴测试**：**相交帧数 0，最近距离 0.83 m。**

**但修好它，暴露出下一个问题 —— 这才是这一节最该说的。** 真正阻止碰撞之后，产线就堵死：

| 避让判据 | 任务 | 送达 | 重叠 |
|---|---:|---:|---:|
| 车心距 + 0.34 m 锥形（错误写法） | 135 | **130** | 1900 |
| 有向矩形 + 宽松互让 | 19 | 12 | 0 |
| 有向矩形 + 实验阈值 | 5 | 1 | 0 |

那一轮 130 次送达，是**靠车辆互相穿过换来的**。反应式避让（互让 + 停靠租约）在这个负载下不够用；
...
真正限制 Gazebo 的是**会车成本**，不是物理：通道净宽 1.45 m 对 1.04 m 的
会车需求，密度一高就会车频繁 —— 见 <a href="#结果">结果</a> 里的表。
