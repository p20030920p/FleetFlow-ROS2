...
> **录制于 2026-09-14**，用当前构建：逻辑栈 + **6 台车 + 40 件物料**，取长运行里
> 活动最密集的 **60 秒**、压成 90 帧、离屏渲染。它替换掉一张**修复之前**的录制 ——
> 那一版里车辆的通过距离在物理下是不允许的，已从仓库移除。这里画的是理想运动学，
> 不是 Gazebo 接触。
>
> 这一段的选取标准是**活动度**而不是最高产量：`tools/pick_active.py` 挑全队位移最大的
> 60 秒，`tools/check_gif.py` 负责验收。中间有一版被丢弃 —— 它通过了"帧帧不同"的
> 幼稚检查，实际几乎不动（帧间变化像素中位 0.077%，本版 0.436%）。
