...
<p align="center">
  <img src="./assets/readme/demo.gif" width="100%" alt="一次完整搬运循环：任务以空心方框出现，拍卖逐单派车，车辆沿规划路径驶向取货位再送达卸货位，完成数上升，物料流转计数从空筒推进到生条、熟条、粗纱成品">
</p>

<p align="center">
  <sub><b>一次完整循环：从发放任务到送达。</b>任务出现 · 拍卖派车 · 车辆沿规划器给出的真实路径行驶
  （实线=已行驶，虚线=剩余）· 计数推进。画面取自真实运行，6 倍速播放。</sub>
</p>
...
六项代价如果每项都不挣工资，这套复杂度就不值得。所以逐项置零、其余不动、同一组种子、同一座工厂，重跑。

<p align="center">
  <img src="./assets/readme/ablation.png" width="100%" alt="留一法消融：完整 CA-SSI 代价与四个各去掉一项的变体，在吞吐与单任务里程上的对比">
</p>

| 代价模型 | 吞吐/分 | 相对完整模型 | 每单行驶（米） | 平均时延（秒） |
| --- | ---: | ---: | ---: | ---: |
...
**真正扛事的是两项。** 拥塞项值 **20% 的吞吐**：去掉它，拍卖就退回到"把车派往已经被占用的工位"，
而这正是这套代价函数存在的全部理由。电量可达项值 **16%** —— 这个不那么直观，
而且在本次这种温和的 0.10 %/m 耗电下依然成立，因为一台车接下它跑不完的任务后，
必须中途脱离去充电，那一单还得重新派人。

另外两项在**这个工况下**是如实的零结果。负载均衡没有可测量的影响 —— 在一轮 120 秒的运行里，
没有哪台车能积累出有意义的里程优势，这不奇怪。任务老化同样没有影响，
而对一个防饿死项来说这正是正确的结果：它的职责是兜住最坏情况，不是抬高平均值。
两项都保留，因为它们要防的那个条件（长班次、角落里的一条低优先级任务）
并不是这组实验能造出来的条件。
...
- 上面的消融是"单一工况、单一车队规模"下的留一法。它说明的是**这座工厂里**哪几项在扛事，
  并不能证明换个厂区还是同样的分摊。负载均衡与任务老化在这里是平的，两项都保留了下来，
  因为它们要防的是这组实验造不出来的条件 —— 见消融一节。
...
在没有独立显卡的机器上，Gazebo 会把实时因子压到 1 以下，于是每一个墙钟指标 ——
吞吐、时延、看门狗超时 —— 都会被一个与分配策略无关的系数缩放。
把对比实验放到无渲染模式下跑，才能保证"被测变量"是唯一的变量。

完整栈是能跑的，数字直接写出来省得猜：4 台 AGV、24 件在制物料，
200 秒墙钟得到 **35 次派单、5 次送达、0 报错，空闲内存全程不低于 6.9 GB**。
它大约比实时慢五倍 —— 这正是它不能当测量台架的原因。这里有一条值得记下来的坑，
因为它很容易被无意间复现：世界里的 4 路相机原先设了 `always_on`，
于是**不管有没有订阅者**，Gazebo 每帧都在离屏渲染 1280x800 的图像。
仅这一项就足以把整轮运行拖到一单都送不完，并把常驻内存推到 OOM killer 的门口。
现在相机是 `always_on=0`（有订阅者才渲染），`factory.launch.py` 默认也不桥接它们；
空跑的开销随之降下来，而 `tools/capture_views.py` 不用改 —— 订阅这个动作本身就会唤醒传感器。

Gazebo 仍然是所有物理相关部分的基准：接触、里程计、LiDAR、多机生成与 TF 布局，
以及上面「车间」一节里的那些渲染图。想交互式跑完整栈，请把 `num_robots` 调小，并接受慢动作。
