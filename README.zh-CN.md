...
**为什么"车心距"是错的判据。** 车体 0.56 × 0.44 m，所以并排需要 0.44 m 车心距、首尾需要 0.56 m，
而任意朝向都不重叠需要 0.712 m。代码里写的是 0.34 m，而且**只在前方 ±52° 锥形里生效** ——
等于主动把车开到必然重叠的距离，同时完全不管侧后方来车。Gazebo 里如实暴露了：
**1900 次重叠、最近 0.259 m、约 0.2 m 的互相穿插。**
现在改成两个有向矩形之间的**分离轴测试**：既不会漏掉真碰撞，也不会挡掉合法的并排通行。
修改后实测：**相交帧数 0，最近距离 0.69 → 1.10 m。** 期间先试过等半径的圆盘判据，
结论是正确的但过于保守 —— Gazebo 里的送达数从 130 掉到 14，因此弃用。
