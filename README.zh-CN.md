...
>
> 仍存在**方差**：同样的命令与时长，产量波动约 2 倍。
> 完整证据：[docs/gazebo-throughput-findings.md](docs/gazebo-throughput-findings.md)。

**限制项是缓冲容量，不是车队也不是通道。** 8 台车 240 s 里 **52% 的时间在 idle**，
单次运输时延中位 13 s。产出塌缩在产线里：17 件料送进梳棉、8 件到并条、2 件到粗纱、2 件成品。
每件料在每个工序都要等加工、等运输、等下游空位，而在制品只有约 12 件 ——
上限约 5 件/分钟，实测 4.6~5.5 件/分钟。车队规模、取放位数量、更大的在途上限均实测无效。
...
*该组数据测于 8 台车、3 种子、80 件物料 —— 是分配研究当初的规模，故保留。当前默认是 4 台车。*
