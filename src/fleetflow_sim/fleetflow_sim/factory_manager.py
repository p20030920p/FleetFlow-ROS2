...
class FactoryManager(Node):
    def __init__(self):
        ...
        # 陈旧任务回收超时。**必须随厂区尺度走**，不能写死：
        # 45 s 是按 32×16 m 的小厂房定的，那时单趟只要十几秒；
        # 厂区放大到 120×60 m 后单趟要 30~60 s，45 s 会把**正在正常行驶**的任务
        # 判成超时并回收 —— 实测 420 s 里回收 34 次，派单 45 单只送达 5 单、成品 0。
        #
        # 按**最长单趟**推算而不是对角线：一趟任务 = 取货 + 送货两段，
        # 每段最坏约厂区长度，再加装卸与等待余量，最后留 2 倍裕度。
        # 对角线的做法会算出 630 s（等于关掉回收），过松。
        # 取值口径：**兜底，不是主检测**。真正卡死的车由控制器自己的
        # 10 s 看门狗先发现（连续两次无进展即放弃任务），回收只负责
        # "车没了/任务没人接"这类没人上报的情况，所以可以取得松。
        # 典型一趟约 60~70 s（单段 40 m / 1.2 m·s⁻¹），拥堵时翻倍到 ~140 s，
        # 于是取 180 s：足够覆盖正常波动，又不至于让死单长期占着在途额度。
        self.declare_parameter("stale_task_timeout_s", 180.0)
        # 投料 / 推进的派单优先级（第 55 节）。
